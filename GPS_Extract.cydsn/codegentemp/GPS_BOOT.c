...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_GPS) || \
         ...
         
{
        ...
        */
        if(0u != GPS_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = GPS_GetRxBufferSize();
                CyDelay(GPS_BYTE2BYTE_TIME_OUT);
            }
            while(GPS_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  GPS_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(GPS_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(GPS_GetRxBufferSize() > 0u)
    {
        tempCount = GPS_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = GPS_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional GPS_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != GPS_PACKET_EOP)
            {
                CyDelay(GPS_WAIT_EOP_DELAY);
            }
        }
