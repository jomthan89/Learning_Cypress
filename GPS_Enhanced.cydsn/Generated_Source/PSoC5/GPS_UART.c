...
uint8 GPS_UART_initVar = 0u;
    ...
    volatile uint8 GPS_UART_txBuffer[GPS_UART_TX_BUFFER_SIZE];
    ...
    uint8 GPS_UART_errorStatus = 0u;
    volatile uint8 GPS_UART_rxBuffer[GPS_UART_RX_BUFFER_SIZE];
    ...
    volatile uint8 GPS_UART_rxBufferOverflow   = 0u;
        ...
        volatile uint8 GPS_UART_rxAddressMode = GPS_UART_RX_ADDRESS_MODE;
...
*  GPS_UART_Start() sets the initVar variable, calls the
*  GPS_UART_Init() function, and then calls the
...
*  and set to one (1u) the first time GPS_UART_Start() is called. This
*  allows for component initialization without re-initialization in all
*  subsequent calls to the GPS_UART_Start() routine.
*
* Reentrant:
*  No.
*
...
*  dialog settings. It is not necessary to call GPS_UART_Init() because
*  the GPS_UART_Start() API calls this function and is the preferred
...
void GPS_UART_Init(void) 
{
            ...
            GPS_UART_errorStatus = 0u;
        #endif /* (GPS_UART_RX_INTERRUPT_ENABLED) */

        #if (GPS_UART_RXHW_ADDRESS_ENABLED)
            GPS_UART_SetRxAddressMode(GPS_UART_RX_ADDRESS_MODE);
            GPS_UART_SetRxAddress1(GPS_UART_RX_HW_ADDRESS1);
            GPS_UART_SetRxAddress2(GPS_UART_RX_HW_ADDRESS2);
...
*  to call GPS_UART_Enable() because the GPS_UART_Start() API
...
void GPS_UART_Enable(void) 
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        #if (GPS_UART_TX_INTERRUPT_ENABLED)
            GPS_UART_ClearPendingTxInt(); /* Clear history of TX_NOT_EMPTY */
            GPS_UART_EnableTxInt();
        #endif /* (GPS_UART_TX_INTERRUPT_ENABLED) */
     #endif /* (GPS_UART_TX_INTERRUPT_ENABLED) */

    #if (GPS_UART_INTERNAL_CLOCK_USED)
        GPS_UART_IntClock_Start();  /* Enable the clock */
    #endif /* (GPS_UART_INTERNAL_CLOCK_USED) */
...
void GPS_UART_Stop(void) 
{
    ...
    #if (GPS_UART_INTERNAL_CLOCK_USED)
        GPS_UART_IntClock_Stop();   /* Disable the clock */
    #endif /* (GPS_UART_INTERNAL_CLOCK_USED) */
...
uint8 GPS_UART_ReadControlRegister(void) 
{
    ...
    #endif /* (GPS_UART_CONTROL_REG_REMOVED) */
}
...
void  GPS_UART_WriteControlRegister(uint8 control) 
{
    ...
    *      GPS_UART_RX_STS_FIFO_NOTEMPTY    Interrupt on byte received.
    *      GPS_UART_RX_STS_PAR_ERROR        Interrupt on parity error.
    *      GPS_UART_RX_STS_STOP_ERROR       Interrupt on stop error.
    *      GPS_UART_RX_STS_BREAK            Interrupt on break.
    *      GPS_UART_RX_STS_OVERRUN          Interrupt on overrun error.
    *      GPS_UART_RX_STS_ADDR_MATCH       Interrupt on address match.
    *      GPS_UART_RX_STS_MRKSPC           Interrupt on address detect.
    *
    * Return:
    *  None.
    *
    ...
    *  GPS_UART_rxBufferLoopDetect - cleared if loop condition was detected
    *     in RX ISR.
    *
    * Reentrant:
    *  No.
    ...
    *******************************************************************************/
    uint8 GPS_UART_ReadRxData(void) 
    {
        ...
        locRxBufferRead  = GPS_UART_rxBufferRead;
        locRxBufferWrite = GPS_UART_rxBufferWrite;

        if( (GPS_UART_rxBufferLoopDetect != 0u) || (locRxBufferRead != locRxBufferWrite) )
        {
            rxData = GPS_UART_rxBuffer[locRxBufferRead];
            locRxBufferRead++;

            if(locRxBufferRead >= GPS_UART_RX_BUFFER_SIZE)
            {
                locRxBufferRead = 0u;
            }
            /* Update the real pointer */
            GPS_UART_rxBufferRead = locRxBufferRead;
            ...
            if(GPS_UART_rxBufferLoopDetect != 0u)
            {
                ...
                #endif /* ((GPS_UART_RX_INTERRUPT_ENABLED) && (GPS_UART_FLOW_CONTROL != 0u)) */
            }
        }
        else
    ...
    *  GPS_UART_RX_STS_FIFO_NOTEMPTY.
    *  GPS_UART_RX_STS_FIFO_NOTEMPTY clears immediately after RX data
    *  register read.
    *
    * Global Variables:
    ...
    *
    * Summary:
    *  Returns the last received byte of data. GPS_UART_GetChar() is
    ...
    *******************************************************************************/
    uint16 GPS_UART_GetByte(void) 
    {
        ...
        /* Protect variables that could change on interrupt */
        GPS_UART_DisableRxInt();
        locErrorStatus = (uint16)GPS_UART_errorStatus;
        GPS_UART_errorStatus = 0u;
        GPS_UART_EnableRxInt();
        return ( (uint16)(locErrorStatus << 8u) | GPS_UART_ReadRxData() );
    #else
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        {
            size = (GPS_UART_RX_BUFFER_SIZE - GPS_UART_rxBufferRead) + GPS_UART_rxBufferWrite;
        }
    ...
    *   GPS_UART_TX_STS_COMPLETE        Interrupt on TX byte complete
    *   GPS_UART_TX_STS_FIFO_EMPTY      Interrupt when TX FIFO is empty
    *   GPS_UART_TX_STS_FIFO_FULL       Interrupt when TX FIFO is full
    *   GPS_UART_TX_STS_FIFO_NOT_FULL   Interrupt when TX FIFO is not full
    *
    * Return:
    *  None.
    *
    ...
    *******************************************************************************/
    void GPS_UART_WriteTxData(uint8 txDataByte) 
    {
        ...
        if(GPS_UART_initVar != 0u)
        {
            ...
            {
                if(GPS_UART_txBufferWrite >= GPS_UART_TX_BUFFER_SIZE)
                {
    ...
    *******************************************************************************/
    void GPS_UART_PutChar(uint8 txDataByte) 
    {
        ...
        */
        ...
        #if ((GPS_UART_TX_BUFFER_SIZE > GPS_UART_MAX_BYTE_VALUE) && (CY_PSOC3))
            /* Disable TX interrupt to protect variables from modification */
            GPS_UART_DisableTxInt();
        #endif /* (GPS_UART_TX_BUFFER_SIZE > GPS_UART_MAX_BYTE_VALUE) && (CY_PSOC3) */

            locTxBufferWrite = GPS_UART_txBufferWrite;
            locTxBufferRead  = GPS_UART_txBufferRead;
        ...
        while( (locTxBufferWrite < locTxBufferRead) ? (locTxBufferWrite == (locTxBufferRead - 1u)) :
                                ...
                                (uint8)(GPS_UART_TX_BUFFER_SIZE - 1u)) );
        ...
        {
            if(locTxBufferWrite >= GPS_UART_TX_BUFFER_SIZE)
            {
                locTxBufferWrite = 0u;
            }
            /* Add to the software buffer. */
            GPS_UART_txBuffer[locTxBufferWrite] = txDataByte;
            locTxBufferWrite++;
            ...
            GPS_UART_txBufferWrite = locTxBufferWrite;

        #if ((GPS_UART_TX_BUFFER_SIZE > GPS_UART_MAX_BYTE_VALUE) && (CY_PSOC3))
            GPS_UART_EnableTxInt();
        #endif /* (GPS_UART_TX_BUFFER_SIZE > GPS_UART_MAX_BYTE_VALUE) && (CY_PSOC3) */

            if(0u != (GPS_UART_TXSTATUS_REG & GPS_UART_TX_STS_FIFO_EMPTY))
            {
                /* Trigger TX interrupt to send software buffer */
                GPS_UART_SetPendingTxInt();
            }
        }
    ...
    *******************************************************************************/
    void GPS_UART_PutString(const char8 string[]) 
    {
        ...
        if(GPS_UART_initVar != 0u)
        {
            ...
            {
                GPS_UART_PutChar((uint8)string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        if(GPS_UART_initVar != 0u)
        {
            ...
            {
                GPS_UART_PutChar(string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        {
            size = (GPS_UART_TX_BUFFER_SIZE - GPS_UART_txBufferRead) +
                    GPS_UART_txBufferWrite;
        }
    ...
    *   GPS_UART_SEND_BREAK - Initialize registers for break, send the Break
    *       signal and return immediately.
    *   GPS_UART_WAIT_FOR_COMPLETE_REINIT - Wait until break transmission is
    *       complete, reinitialize registers to normal transmission mode then return
    *   GPS_UART_REINIT - Reinitialize registers to normal transmission mode
    *       then return.
    *   GPS_UART_SEND_WAIT_REINIT - Performs both options: 
    *      GPS_UART_SEND_BREAK and GPS_UART_WAIT_FOR_COMPLETE_REINIT.
    ...
    *     SendBreak(0);     - initialize Break signal transmission
    *         Add your code here to use CPU time
    *     When interrupt appear with GPS_UART_TX_STS_COMPLETE status:
    *     SendBreak(2);     - complete Break operation
    *
    * Side Effects:
    *  The GPS_UART_SendBreak() function initializes registers to send a
    *  break signal.
    ...
    void GPS_UART_SendBreak(uint8 retMode) 
    {
        ...
        if(GPS_UART_initVar != 0u)
        {
                ...
                (retMode == GPS_UART_SEND_WAIT_REINIT) )
            {
                ...
                #if(GPS_UART_TXCLKGEN_DP)
                    txPeriod = GPS_UART_TXBITCLKTX_COMPLETE_REG;
                    GPS_UART_TXBITCLKTX_COMPLETE_REG = GPS_UART_TXBITCTR_BREAKBITS;
                #else
                    txPeriod = GPS_UART_TXBITCTR_PERIOD_REG;
            ...
            #if(GPS_UART_TXCLKGEN_DP)
                GPS_UART_TXBITCLKTX_COMPLETE_REG = txPeriod;
            #else
                GPS_UART_TXBITCTR_PERIOD_REG = txPeriod;
            #endif /* End GPS_UART_TXCLKGEN_DP */
    ...
    * Parameters:
    *  addressMode: 
    *       GPS_UART_SET_SPACE - Configure the transmitter to send the next
    *                                    byte as a data.
    *       GPS_UART_SET_MARK  - Configure the transmitter to send the next
    *                                    byte as an address.
    *
    * Return:
    *  None.
    *
    * Side Effects:
    *  This function sets and clears GPS_UART_CTRL_MARK bit in the Control
    *  register.
    *
    *******************************************************************************/
