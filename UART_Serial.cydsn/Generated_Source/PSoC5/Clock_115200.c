...
/*******************************************************************************
* Function Name: Clock_115200_Start
********************************************************************************
*
* Summary:
...
void Clock_115200_Start(void) 
{
    ...
    Clock_115200_CLKEN |= Clock_115200_CLKEN_MASK;
	Clock_115200_CLKSTBY |= Clock_115200_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_115200_Stop
********************************************************************************
*
* Summary:
...
void Clock_115200_Stop(void) 
{
    ...
    Clock_115200_CLKEN &= (uint8)(~Clock_115200_CLKEN_MASK);
	Clock_115200_CLKSTBY &= (uint8)(~Clock_115200_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_115200_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_115200_CLKEN & Clock_115200_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_115200_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_115200_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_115200__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_115200_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_115200_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_115200_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_115200_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_115200_GetSourceRegister();
    uint16 oldDivider = Clock_115200_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_115200_CLKEN & Clock_115200_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_115200_DIV_PTR, clkDivider);
                Clock_115200_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_115200_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_115200_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_115200_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_115200_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_115200_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_115200_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_115200_MOD_SRC |= modeBitMask & (uint8)Clock_115200_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_115200_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_115200_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_115200_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_115200_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_115200_MOD_SRC & (uint8)(Clock_115200_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_115200_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_115200_GetDividerRegister();
    uint8 oldSrc = Clock_115200_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_115200_MOD_SRC |= CYCLK_SSS;
        Clock_115200_MOD_SRC =
            (Clock_115200_MOD_SRC & (uint8)(~Clock_115200_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_115200_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_115200_MOD_SRC & Clock_115200_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_115200_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_115200_PHASE = clkPhase & Clock_115200_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_115200_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_115200_PHASE & Clock_115200_PHASE_MASK;
}
