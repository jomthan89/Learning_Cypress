...
#if defined(GPS_BUART_sCR_SyncCtl_CtrlReg__CONTROL_REG)
...
/* Only if RX is enabled */
        ...
        #define GPS_EnableRxInt()  CyIntEnable (GPS_RX_VECT_NUM)
        #define GPS_DisableRxInt() CyIntDisable(GPS_RX_VECT_NUM)
    ...
    uint16 GPS_GetRxBufferSize(void)
                                                            ;
...
/* Only if TX is enabled */
        ...
        #define GPS_EnableTxInt()  CyIntEnable (GPS_TX_VECT_NUM)
        #define GPS_DisableTxInt() CyIntDisable(GPS_TX_VECT_NUM)
        #define GPS_SetPendingTxInt() CyIntSetPending(GPS_TX_VECT_NUM)
        #define GPS_ClearPendingTxInt() CyIntClearPending(GPS_TX_VECT_NUM)
...
/* Communication bootloader APIs */
    ...
    #define GPS_PACKET_EOP         (0x17u) /* End of packet defined by bootloader */
    #define GPS_WAIT_EOP_DELAY     (5u)    /* Additional 5ms to wait for End of packet */
    #define GPS_BL_CHK_DELAY_MS    (1u)    /* Time Out quantity equal 1mS */
...
#define GPS_BIT_CENTER                         (GPS_OVER_SAMPLE_COUNT - 2u)
...
extern uint8 GPS_initVar;
    ...
    extern volatile uint8 GPS_txBuffer[GPS_TX_BUFFER_SIZE];
    ...
    extern uint8 GPS_errorStatus;
    extern volatile uint8 GPS_rxBuffer[GPS_RX_BUFFER_SIZE];
    extern volatile uint16 GPS_rxBufferRead;
    extern volatile uint16 GPS_rxBufferWrite;
...
#if (GPS_RXHW_ADDRESS_ENABLED)
    #define GPS_RX_ADDRESS_MODE    (0u)
    #define GPS_RX_HW_ADDRESS1     (0u)
    #define GPS_RX_HW_ADDRESS2     (0u)
#endif /* (GPS_RXHW_ADDRESS_ENABLED) */
...
#define GPS_WAIT_1_MS      GPS_BL_CHK_DELAY_MS   

#define GPS_TXBUFFERSIZE   GPS_TX_BUFFER_SIZE
#define GPS_RXBUFFERSIZE   GPS_RX_BUFFER_SIZE

#if (GPS_RXHW_ADDRESS_ENABLED)
    #define GPS_RXADDRESSMODE  GPS_RX_ADDRESS_MODE
    #define GPS_RXHWADDRESS1   GPS_RX_HW_ADDRESS1
    #define GPS_RXHWADDRESS2   GPS_RX_HW_ADDRESS2
