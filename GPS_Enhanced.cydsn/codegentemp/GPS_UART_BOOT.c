...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_GPS_UART) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        GPS_UART_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != GPS_UART_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = GPS_UART_GetRxBufferSize();
                CyDelay(GPS_UART_BYTE2BYTE_TIME_OUT);
            }
            while(GPS_UART_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  GPS_UART_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(GPS_UART_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(GPS_UART_GetRxBufferSize() > 0u)
    {
        tempCount = GPS_UART_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = GPS_UART_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional GPS_UART_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != GPS_UART_PACKET_EOP)
            {
                CyDelay(GPS_UART_WAIT_EOP_DELAY);
            }
        }
